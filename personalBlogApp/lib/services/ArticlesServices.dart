...
class Articlesservices {
  ...
  Future<List<ArticlesModel>>getArticlesAgainstCategory(String categoryName) async {
    try{
      final response = await http.get(Uri.parse("${Servicesendpoints.ARTICLES_URL}?category__category_name__icontains=$categoryName"));
      if (response.statusCode == 200){
        // decode the response
        List<dynamic> data = json.decode(response.body);
        return ArticlesModel.fromJsonList(data);
        ...
        throw Exception("Failed to load the articles");
      }
    }
    catch (e){
      throw Exception(e);
